...
import { Box, Typography, Alert, Backdrop, CircularProgress } from '@mui/material';
import {
...
export const RestaurantsListPage = memo(() => {
  ...
  const campaignModal = useDialogState<{ restaurantId: string; restaurantName: string; campaigns: RestaurantCampaign[] }>();
  ...
  const [, setLoadingFilterData] = useState(false);
  
  // Load filter dictionaries when filter opens
  const loadFilterDictionaries = useCallback(async () => {
    if (filterDictionaries) return; // Already loaded
    ...
    try {
      ...
      setFilterDictionaries(data);
    } catch (error) {
      logger.error('Failed to load filter dictionaries', error as Error);
    } finally {
      setLoadingFilterData(false);
    }
  }, [filterDictionaries]);
  
  // Load campaign modal data when modal opens
  const loadCampaignModalData = useCallback(async () => {
    try {
      const [
        advertisersData,
        slotsData,
        ...
        (async () => {
          ...
          return await schedulesApi.list();
        })(),
      ]);

      setAdvertisers(advertisersData);
      setAllDictionaries({
        cities: filterDictionaries?.cities as any || [],
        locations: filterDictionaries?.districts as any || [],
        restaurantTypes: filterDictionaries?.restaurantTypes || [],
        priceSegments: filterDictionaries?.priceSegments || [],
        menuTypes: filterDictionaries?.menuTypes || [],
        slots: slotsData,
        schedules: schedulesData,
      });
    } catch (error) {
      logger.error('Failed to load campaign modal data', error as Error);
    }
  }, [setAdvertisers, setAllDictionaries, filterDictionaries]);
  ...
  ], [integrationTypes, getDisplayName, t]);
  ...
  const { isLoading: isLoadingEdit, handleEdit: handleCampaignTargeting } = useEditWithLoading<RestaurantListItem>({
    entityName: 'restaurant campaigns',
    fetchById: async (id) => {
      // Load campaign modal data (campaigns + advertisers)
      await loadCampaignModalData();
      // Fetch restaurant campaigns and validate before opening modal
      const campaigns = await restaurantsApi.getRestaurantCampaigns(id as string);
    ...
    getEntityId: (restaurant) => restaurant.id,
  });
  ...
  const handleOpenFilterDrawer = useCallback(() => {
    loadFilterDictionaries(); // Load filter data when opening
    filterDrawer.open();
  }, [filterDrawer, loadFilterDictionaries]);
  ...
  return (
    <Box>
      ...
      <RestaurantCampaignsModal
        ...
        restaurantCampaigns={campaignModal.data?.campaigns || []}
      />
    </Box>
  );
});
