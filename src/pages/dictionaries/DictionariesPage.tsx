...
export const DictionariesPage = memo(() => {
  ...
  const handleEdit = useCallback(async (id: string | number) => {
    ...
    try {
      // Re-fetch the list to ensure we have fresh data
      const freshList = await dictionariesApi.list(dictKey as DictionaryKey);
      const item = freshList.find((i) => i.id === String(id));
      
      if (item) {
        formDialog.openDialog(typeof id === 'string' ? parseInt(id, 10) : id);
      } else {
        enqueueSnackbar(t('common.error.notFound'), { variant: 'error' });
      }
    } catch (error) {
      enqueueSnackbar(t('common.error.loadFailed'), { variant: 'error' });
    }
  }, [formDialog, dictKey, enqueueSnackbar, t]);
