...
export class ApiError extends Error {
  ...
  getUserMessage(): string {
    ...
    if (this.errorMessage) {
      ...
      if (this.errorCode) {
        return `${this.errorMessage} (Code: ${this.errorCode})`;
      }
      return this.errorMessage;
    }

    // Return default message based on status code with code
    const defaultMessage = ApiError.getDefaultMessage(this.statusCode);
    return `${defaultMessage} (Code: ${this.statusCode})`;
  }
