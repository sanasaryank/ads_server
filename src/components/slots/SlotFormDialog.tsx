...
export const SlotFormDialog = memo(({
  ...
  // Fetch slot data in edit mode
  const { data: slotData, loading: slotLoading } = useFetch(
    async () => {
      if (!isEditMode || !open) return null;
      try {
        return await slotsApi.getById(slotId);
      } catch (error) {
        logger.error('Error loading slot', error as Error, { slotId });
        throw error;
      }
    },
    [open, isEditMode, slotId]
  );
