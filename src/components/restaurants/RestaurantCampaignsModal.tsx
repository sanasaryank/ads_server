...
import { useMultilingualName, useDialogState, useDataSelection } from '../../hooks';
...
export const RestaurantCampaignsModal = memo(({
  ...
  // Compute selected campaigns from props - this updates when restaurantCampaigns changes
  const initialSelectedCampaigns = useMemo(() => {
  ...
  // State for selected campaigns - initialize with computed value
  const [selectedCampaigns, setSelectedCampaigns] = useState<Map<string, Map<string, { schedules: string[]; placements: string[] }>>>(initialSelectedCampaigns);
  
  // Sync selected campaigns when data changes (new restaurant or data refresh)
  useEffect(() => {
