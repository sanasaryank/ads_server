...
import { PlacementImageDisplay } from './PlacementImageDisplay';
...
export const PlacementsDialog = memo<PlacementsDialogProps>(function PlacementsDialog({
  ...
  const handleSave = useCallback(() => {
    ...
    onClose();
  }, [onSave, onClose, selection.selectedArray]);

  const handleCancel = useCallback(() => {
    selection.setSelection(selectedPlacements);
    onClose();
  }, [onClose, selectedPlacements, selection]);
  ...
  return (
    <Dialog open={open} onClose={handleCancel} maxWidth="sm" fullWidth disableRestoreFocus container={() => document.getElementById('modal-root')}>
      <ErrorBoundary>
                  ...
                  <AccordionDetails>
                    <Box sx={{ pl: 4, display: 'flex', flexDirection: 'column', gap: 1 }}>
                      {(placement.dishes || []).map((dish) => (
                        <Box key={dish.id} sx={{ py: 0.5, display: 'flex', gap: 2, alignItems: 'flex-start' }}>
                          <Box sx={{ flexShrink: 0 }}>
                            <PlacementImageDisplay 
                              imageData={dish.imageData && dish.imageData.length > 0 ? dish.imageData[0] : null} 
                              rowHeight={60} 
                            />
                          </Box>
