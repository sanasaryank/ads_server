...
import type { RestaurantListItem, DictionaryItem } from '../../types';
...
export const RestaurantInfoPopover = memo(({
  ...
  const { cities, locations, restaurantTypes, menuTypes, priceSegments } = useDictionariesStore();
  ...
  const getCityName = useCallback((id: string) => {
  ...
  const getItemName = useCallback((items: DictionaryItem[], id: string) => {
    const item = items.find((item) => String(item.id) === String(id));
    return item ? getDisplayName(item.name) : '-';
  }, [getDisplayName]);

  const getItemNames = (items: DictionaryItem[], ids: string | string[]) => {
